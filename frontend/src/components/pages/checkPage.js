...
class CheckPage extends Component {
  ...
  getStartTime = () => {
    if (!this.state.result || !this.state.result.startTime) {
      return "Loading...";
    }
    const date = new Date(this.state.result.startTime);
  ...
  renderForth = () => {
    ...
    for (let i = 0; i < 5; i++) {
      allCol.push(<div className="row">{prize.slice(i*10, (i+1)*10).map(ele => this.renderSmallNumber(ele))}</div>);
    }
    return allCol
  }
    ...
    return (
      <div>
        ...
        <div className="container">
          ...
          <div className="row">
            <div className="col-4 offset-4 date-txt">{this.getStartTime()}</div>
          </div>
        </div>
          ...
          <div className="row">
            <div className="col-3">
              <div className="card text-center">
                ...
                <div className="card-footer text-muted">6,000,000฿ / 1 prize</div>
              </div>
            </div>
